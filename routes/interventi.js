...
router.post("/add_intervento", (req, res) => {
  ...
  if (!ore_utilizzate || isNaN(ore_utilizzate) || Number.parseFloat(ore_utilizzate) <= 0) {
    return res.status(400).json({ error: "Ore utilizzate deve essere un numero maggiore di 0" })
  }
    ...
    db.run(
      ...
      function (err) {
        if (err) {
          ...
          return res.status(500).json({ error: "Errore nell'inserimento dell'intervento: " + err.message })
        }
        ...
        db.run(
          ...
          (err2) => {
            if (err2) {
              ...
              return res.status(500).json({ error: "Errore nell'aggiornamento delle ore cliente: " + err2.message })
            }
      ...
      db.run(
        ...
        (err2) => {
          ...
          db.run("UPDATE clienti SET ore_residue = ? WHERE id = ?", [nuove_residue, row.cliente_id], (err3) => {
            ...
            res.json({ success: true, message: "Intervento aggiornato con successo" })
          })
        },
      )
    },
