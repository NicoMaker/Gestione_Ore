...
function updateClientiTable(clienti) {
  ...
  clienti.forEach((cliente) => {
    const oreUtilizzate = (
      cliente.ore_acquistate - cliente.ore_residue
    ).toFixed(1);
    const percentualeUsata =
      ((cliente.ore_acquistate - cliente.ore_residue) /
...
async function handleClienteSubmit(e) {
  ...
  if (
    !oreAcquistate ||
    isNaN(oreAcquistate) ||
    Number.parseFloat(oreAcquistate) <= 0
  ) {
...
async function salvaCliente(id, button) {
  ...
  if (
    !data.ore_acquistate ||
    isNaN(data.ore_acquistate) ||
    Number.parseFloat(data.ore_acquistate) <= 0
  ) {
    showAlert("Ore acquistate deve essere un numero maggiore di 0", "error");
    return;
  }
  ...
  const oreAcquistateInput = row.querySelector(
    'input[data-field="ore_acquistate"]',
  );
  ...
  try {
    ...
    if (response.ok) {
      ...
      showAlert(
        result.error || "Errore nell'aggiornamento del cliente",
        "error",
      );
    }
  } catch (error) {
...
function eliminaCliente(id) {
  ...
  const ragioneSociale =
    row?.querySelector('input[data-field="ragione_sociale"]')?.value?.trim() ||
    "questo cliente";
...
async function deleteCliente(id) {
  try {
    ...
    if (response.ok) {
      ...
      showAlert(
        result.error || "Errore nell'eliminazione del cliente",
        "error",
      );
    }
  } catch (error) {
